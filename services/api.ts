...
export const api = {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if (session?.user) {
        const { data: userProfile } = await supabase
          ...
          .eq('id', session.user.id)
          .single();
        callback(userProfile ? toUser(userProfile, session.user.email) : null);
      } else {
        callback(null);
      }
    });
  ...
  createUser: async(userData: Omit<User, 'id' | 'role'> & {password: string}): Promise<User> => {
    // This is a client-side function for user self-signup
    ...
    // Fix: Implement missing admin and user management functions
    // NOTE: Admin actions like creating/deleting other users should be done via secure Edge Functions,
    // not directly from the client. The implementation below simulates a call to such a function.

    deleteUser: async (userId: string): Promise<void> => {
        // In a real app, this MUST be an edge function call for security reasons.
        // e.g., const { error } = await supabase.functions.invoke('delete-user', { body: { userId } });
        const { error } = await supabase.from('users').delete().eq('id', userId); // Simplified for this context
        if (error) throw error;
    },
    ...
    createAdmin: async (userData: Omit<User, 'id' | 'role'> & { password: string }): Promise<User> => {
        ...
        const { data: authData, error: authError } = await supabase.auth.signUp({
            ...
            password: userData.password,
            options: {
                data: { name: userData.name, role: Role.PROGRAMADOR }
            }
        });
        if (authError) throw authError;
        if (!authData.user) throw new Error("Criação de Admin falhou.");
        return {
            id: authData.user.id,
            email: authData.user.email!,
            name: userData.name,
    ...
    getStudentProgress: async (userId: string, courseId: string): Promise<string[]> => {
        ...
        if (error) throw error;
        // Fix: The type from Supabase for a JSONB array can be `unknown[]`. A double cast is needed to assert it as `string[]`.
        return (data.completed_lesson_ids as unknown as string[]) || [];
    },
    ...
    getStudentProgressOverview: async (): Promise<{ student: User; course: Course; progress: number; }[]> => {
        ...
        return data.map(e => {
            ...
            return {
                // Fix: `e.users` is inferred as an array from the join. Access the first element for the user object.
                student: toUser(e.users[0], e.users[0].email),
                course,
                progress
            }
        });
